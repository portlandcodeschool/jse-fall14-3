...
var cardReader = {
	...
	cardID: function(rank,suit) {
	    return (cardReader.rank(card)) + (cardReader.suit(card));
	},
	color: function(card) {
	    return (cardReader.suit(card) < 3)? "red": "black";
	},
	name: function(card) {
    	return rankNames[(cardReader.rank(card))]+' of '+suitNames[(cardReader.suit(card))];
	}
};
